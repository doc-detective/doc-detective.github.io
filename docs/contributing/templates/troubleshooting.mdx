...
1. **[Action 1]**: [Explanation] \`\`\`[language if applicable] [Command or code] \`\`\`
...
\`\`\`json title=".doc-detective.json" { "contexts": [ { "app": { "browser": "chrome" } } ] } \`\`\`
...
\`\`\`bash npx doc-detective runTests --input your-test.spec.json \`\`\`
