...
Doc Detective processes DITA map files (`.ditamap`) to organize and structure your documentation tests.

### File path resolution

DITA maps can reference topics in different directory structures:

- Topics in the same directory as the map
- Topics in subdirectories
- Topics in parent directories (using `../` paths)
- Topics in sibling directories

Doc Detective resolves all relative paths, including those requiring parent directory traversal (`../`). If your ditamap references topics with `../` paths, Doc Detective finds a common ancestor directory containing all files, temporarily rewrites paths, and processes the ditamap.
...
<map>
  ...
  <topicref href="../reference/api-overview.dita"/>
</map>
```

Doc Detective processes this ditamap without changes to your file structure or paths.
