...
You can test DITA content from:

- **Local files**: DITA files stored in your project directory
- **Heretto CCMS**: Download and test DITA maps directly from your Heretto instance using the [Heretto integration](/docs/get-started/config/heretto-integration)
...
### DITA `<data>` element format

You can embed test steps using the DITA `<data>` element with the `name="step"` attribute.

```xml
<data name="step">{"checkLink": "https://example.com"}</data>
```

The `<data>` element supports JSON, YAML, or XML attribute formats:

**JSON format:**

```xml
<data name="step">{"click": "Submit"}</data>
```
...
The `<data>` element can appear anywhere inline elements are allowed. This makes it easy to embed test actions directly alongside your documentation content.

Use the `<data>` element when you want test definitions in your DITA content model or when processing instructions and comments don't fit your workflow.
