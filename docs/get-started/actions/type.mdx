...
- **Object Format:** Use an object for more control:
  ...
  - `scope`: (Optional) Named scope of an existing terminal or code session to type into. When you specify this, the action sends input to the scoped process instead of the browser. This is useful for sending commands to long-running processes started with `runShell` or `runCode`. Scope names must be alphanumeric with hyphens or underscores (1-64 characters).
...
When targeting a terminal with `scope`, you can use these additional control keys:

| Key            | Code               | Description                                      |
| -------------- | ------------------ | ------------------------------------------------ |
| Ctrl+C         | $CTRL_C$           | Sends interrupt signal (SIGINT) to the process   |
| Ctrl+D         | $CTRL_D$           | Sends end-of-file (EOF) to the terminal          |
| Ctrl+Z         | $CTRL_Z$           | Sends suspend signal (SIGTSTP) to the process    |

These keys only work when `type` targets a terminal scope.
...
### Type commands into a scoped terminal

This example starts a development server in a scoped terminal and then types a command into that same terminal.
