...
- **Object Format:** Use an object for more control over the path, visual comparison, and cropping:
  ...
  - `crop`: (Optional) Crop the screenshot to a specific element. Provide a CSS/XPath selector string (for example, `"#myElement"`) or an object with element finding properties:
    - `selector`: (Optional) The [CSS selector](/docs/get-started/selectors/css) or [XPath selector](/docs/get-started/selectors/xpath) of the element to crop to.
    - `elementText`: (Optional) Display text of the element to crop to.
    - `elementId`: (Optional) The ID attribute of the element. Supports regex patterns using `/pattern/` syntax.
    - `elementTestId`: (Optional) The `data-testid` attribute of the element. Supports regex patterns.
    - `elementClass`: (Optional) Class name or array of class names that the element must have.
    - `elementAttribute`: (Optional) Object of attribute key-value pairs that the element must have.
    - `elementAria`: (Optional) The computed accessible name of the element. Supports regex patterns.
    - `padding`: (Optional) Add padding around the element before cropping. Can be a single number (all sides) or an object `{ top, right, bottom, left }`. Padding that extends beyond the viewport is automatically clamped.
