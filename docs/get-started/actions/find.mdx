...
- **String Shorthand:** Provide the display text, CSS/XPath selector, ID, test ID, or ARIA name directly as the value for the `find` key. Doc Detective searches across all these types in parallel and uses the first match found with this precedence: selector → elementText → elementAria → elementId → elementTestId.
