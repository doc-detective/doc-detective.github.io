...
- **Object Format:**
  ...
  - `scope`: (Optional) A named scope for running code in a persistent execution context that you can reuse across multiple steps. Useful for long-running processes or maintaining state between code executions. Scope names must be alphanumeric with hyphens or underscores (1-64 characters). Use the `terminateScope` action to clean up when you're done.
  - `waitUntil`: (Optional) Conditions to wait for before considering the step complete. Useful when running long-running processes or code that needs time to initialize. You can specify patterns to match in stdout or stderr output:
    ...
    - Regex must start and end with `/` (for example, `/Server started/` or `/Ready/`).
...
### Start a Python background process

This example starts a Python process in a persistent scope and waits for it to be ready.
  ...
  "tests": [
    {
      "steps": [
        {
          "description": "Start background process and wait for it to be ready.",
          "runCode": {
            "language": "python",
            "code": "import time\nwhile True:\n    print('Running')\n    time.sleep(1)",
...
This example starts a Node.js HTTP server and waits for the startup message.
