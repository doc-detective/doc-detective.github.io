...
3. Run a test, pointing to the file you just created:
    ...
    <CodeBlock language="bash" title="Run test">
      docker run -v .:/workspace docdetective/docdetective --input /workspace/sample.spec.json
    </CodeBlock>
    {/* step runShell:
    command: "docker run -v .:/workspace docdetective/docdetective --input /workspace/sample.spec.json"
    workingDirectory: "." */}

    :::note

    The `-v` argument mounts your current directory (`.`) to `/workspace` in the container, giving Doc Detective access to your test specification and enabling it to output results and screenshots. If your test specification is hosted at an HTTPS URL, the `-v` argument is not necessary.
