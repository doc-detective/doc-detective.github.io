...
function extractObjectSchemas(schema, parentSchemaId = '', currentPath = '') {
  ...
  if ((schema.type === 'object' || !schema.type) && schema.properties) {
    ...
    if (!schemaRegistry.has(schemaId)) {
      ...
      for (const [propName, propSchema] of Object.entries(schema.properties)) {
        ...
        const nestedSchemas = extractObjectSchemas(propSchema, schemaId, propPath);
  ...
  if (schema.type === 'array' && schema.items) {
    ...
    for (let i = 0; i < itemSchemas.length; i++) {
      ...
      const nestedSchemas = extractObjectSchemas(itemSchemas[i], parentSchemaId, itemPath);
      Object.assign(extractedSchemas, nestedSchemas);
    }
  }
  ...
  ['anyOf', 'oneOf', 'allOf'].forEach(key => {
    if (Array.isArray(schema[key])) {
      schema[key].forEach((subSchema, index) => {
        const subPath = `${currentPath}${currentPath ? '.' : ''}${key}[${index}]`;
        const nestedSchemas = extractObjectSchemas(subSchema, parentSchemaId, subPath);
        Object.assign(extractedSchemas, nestedSchemas);
      });
    }
  });
    ...
    if (!propSchema.type && (propSchema.anyOf || propSchema.oneOf)) {
      ...
      if (options.length > 0) {
        ...
        if (firstOption.type === 'string') {
          example[propName] = firstOption.enum ? firstOption.enum[0] : "example";
        } else if (firstOption.type === 'number' || firstOption.type === 'integer') {
          example[propName] = 42;
...
function getTypeString(schema) {
  ...
  if (schema.anyOf || schema.oneOf) {
    ...
    // Multiple options
    return "one of:<br/>" + options.map(option => {
...
function getArrayItemTypeString(items) {
  ...
  if (!Array.isArray(items)) {
    ...
    if (items.anyOf || items.oneOf) {
      ...
      return "one of: " + options.map(option => getTypeString(option)).join(", ");
    }
  ...
  for await (const key of schemasToGenerate) {
    ...
    for (const [schemaId, _] of Object.entries(extractedSchemas)) {
      ...
      if (schemaId.includes('-')) {
        // For nested schemas without titles
        fileName = `${createValidFileName(schemaId)}.md`;
      } else {
